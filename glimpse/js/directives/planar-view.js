      ...
      scope.$watch('atoms', function (atoms) {
        ...
        force.links(force.links().filter(function (n) {
          for (var i = 0; i < graph.edges.length; i++) {
            if (n.source && n.target)
              if (graph.edges[i].source == n.source.handle && graph.edges[i].target == n.target.handle)
                return true;
          }
          return false;
        }));
        ...
        node.on("click", function (sender) {
          ...
          if(scope.tool == 'center'){
            ...
            edge.style("opacity", function (d) {
              return (nodeHandlesToShow.indexOf(d["source"]["handle"]) > -1 &&
                nodeHandlesToShow.indexOf(d["target"]["handle"]) > -1) ? 1 : 0.05;

            });
          }

          if(scope.tool == 'slidemode')
          {
