...
glimpse.controller("mainCtrl", function ($rootScope, $scope, $window, $timeout, 
  ...
  var toolboxPanel, atomDetailsPanel, terminalPanel, threeDPanel, jsonPanel,
    planarPanel, schemePanel, tabularPanel,  filtersPanel, settingsPanel,
    addNodePanel, connectPanel;
  var documentManagerNode, toolboxNode, jsonNode, threeDNode, terminalNode,
    planarNode, atomDetailsNode,settingsDialog, filtersDialog, addNodeDialog,
    connectDialog;
  ...
  $scope.showPanel = function (panel) {
    if (panel == 'filters') {
      ...
      filtersDialog.setPosition(window.innerWidth - filtersPanel._cachedWidth,
        window.innerHeight - filtersPanel._cachedHeight);

    } else if (panel == 'planar_settings') {
      settingsDialog = new dockspawn.Dialog(settingsPanel, dockManager);
      settingsDialog.setPosition(window.innerWidth - settingsPanel._cachedWidth,
        window.innerHeight - settingsPanel._cachedHeight);
    }

    else if (panel == 'add_node') {
      addNodeDialog = new dockspawn.Dialog(addNodePanel, dockManager);
      addNodeDialog.setPosition((window.innerWidth - addNodePanel._cachedWidth) / 2,
        (window.innerHeight - connectPanel._cachedHeight) / 2);

    } else if (panel == 'connect') {
      connectDialog = new dockspawn.Dialog(connectPanel, dockManager);
      connectDialog.setPosition((window.innerWidth - connectPanel._cachedWidth) / 2,
        (window.innerHeight - connectPanel._cachedHeight) / 2);
    }
  };
  ...
  $window.onload = function () {
    ...
    // Create PanelContainers using the dockspawn library
